...
function saveSvg() {
  // 使用 save() 函数保存 SVG 文件
  save('text3.svg'); // 自定义文件名
}
function setup() {
  createCanvas(900,490,SVG);
  background(255);
...
akl.push(font.textToPoints(q[i], 120*i, 320, 200, { sampleFactor: 0.8 }))
...
akl.forEach(item=>{
  ...
  for (let i = 0; i < 20; i+=5) {
    ...
    if(item[i]){
      balls.push(new Ball(item[i].x+random(-5,5), item[i].y+random(-5,5), random(10, 30),item));
    }
...
push()
  translate (width/2-80-(p.length-1)*58,0)
