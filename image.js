...
let makeOriginalCanvas = function(n) {
  return function(p) {
    ...
    p.setup = function() {
        ...
        .style('width', 33.75 + 'vw') // 容器宽度与画布相同
        .style('height', 21.6+ 'vw') // 容器高度与画布相同
        .style('display', 'flex');// 中心对齐
        let viewportWidth = window.innerWidth; // 获取视口宽度
        let canvasWidth = viewportWidth * 0.3375; // 计算画布宽度
        let canvasHeight = canvasWidth * (21.6 / 33.75); // 根据比例计算画布高度
        
        p.createCanvas(canvasWidth,canvasHeight).parent(container); // 将画布添加到容器中
        ...
        function setButtonStyle(button, width, height) {
          ...
          button.style('border-radius', '50px');        // 圆角边框
          button.style('margin-left', '10px');        // 圆角边框
...
sizeButtonContainer.style('margin-top', '20px');
...
title.style('font-size', '16px'); // 设置字体大小
...
lButton.parent(buttonRow);
    ...
    p.draw = function() {
            // positionFileInput();
            if (imgs[n]) {  // 确保图片已经加载
              p.image(imgs[n], 0, 0, canvasWidth,canvasHeight);
          }
            // p.fill(255,0,0);
            // console.log("what?")
            // p.circle(20,20,30);
...
function adjustRectangleSize(size) {
  ...
  let element = document.getElementById('processingCNV1'); // 选择 ID 为 processingCNV1 的元素
  let element2 = document.getElementById('container1'); // 选择 ID 为 processingCNV1 的元素
  element2.innerHTML=""
  if (element) {
    element.innerHTML = ''; 
  } else {
  ...
  knm[0]=new p5(makeProcessedCanvas(1), 'container1')




  // let element = document.getElementById('container1'); // 选择 ID 为 processingCNV1 的元素
  // if (element) {
...
let makeProcessedCanvas = function(n) {
  ...
  p.setup = function() {
    ...
    let canvasHeight = canvasWidth * (21.6 / 33.75); // 根据比例计算画布高度
      ...
      cnv = p.createCanvas(563,canvasHeight,p.SVG); 
      ...
      bigButtonContainer.style('margin-top', '20px'); 
      ...
      colorContainer.style('font-size', '16px'); 
...
for (let i = 0; i < 1; i++) { // Loop to create 6 groups
